...
Protected/core items are assumed protected for loadout planning. High Risk mode protects none. All other selected items count toward the filler-risk budget using RuneLite's current item price, except items physically locked with a Trouver parchment, which use their repair liability. Current high-tier Trouver items use the conservative 500,000 GP mangling fee for deaths above level 20 Wilderness; legacy locked mid-tier items use their normal break-repair fee. This remains a planning estimate, not a prediction of the items kept or lost on an actual Wilderness death.
...
Ordinary equipment with no usable RuneLite market price is excluded from Auto and from the alternatives list rather than being treated as zero-risk. This includes implicit-value items such as the Pendant of Ates and Barrows gloves. An unpriced Trouver-capable item is included only when the exact physical item ID shows that a parchment has actually been applied; an ordinary imbued god cape or Void piece remains excluded.

The committed Trouver table is generated by `scripts/generate_trouver_risk_values.py`. The script joins Jagex's Trouver categories, the current OSRS Wiki `(broken)` repair table, and RuneLite's generated gameval item IDs. It records the source revisions in the generated Java file and refuses incomplete category matches.
